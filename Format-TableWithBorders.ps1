...
.EXAMPLE
PS C:\> Format-TableWithBorders -inputObject $(Get-Process w* | select Handles, NPM, PM, CPU, Id, ProcessName)
...
.EXAMPLE
PS C:\> Get-ChildItem *.log | Select mode,LastWriteTime,Length,Name,VersionInfo | Format-TableWithBorders
...
| -a---- | 10/14/2019 12:55:10 PM |   7885 | 953835-20191014-124219.log | File: C:\953835-20191014-124219.log; InternalNa ... |
+--------+------------------------+--------+----------------------------+-----------------------------------------------------+
...
function Format-TableWithBorders {
    ...
    process {
        # Use the DefaultDisplayPropertySet if the object has one
        if($inputObject.PSStandardMembers.DefaultDisplayPropertySet.ReferencedPropertyNames){
            $selectedProperties = ($inputObject | Select-Object -first 1).PSStandardMembers.DefaultDisplayPropertySet.ReferencedPropertyNames
            $inputObject = $inputObject | Select-Object -property $selectedProperties
        }
